...
void UploadManager::CreateDevice()
{
    ...
    if (FAILED(dxgiDevice->GetAdapter(&dxgiAdapter))) {
        DebugLog::Error(__FUNCTION__, " => QueryInterface from IDXGIDevice1 to IDXGIAdapter failed.");
        return;
    }
...
TexturePtr UploadManager::CreateCompatibleSharedTexture(const TexturePtr& texture)
{
    if (!device_)
    {
        DebugLog::Error(__FUNCTION__, "device has not been created yet.");
        return nullptr;
    }
    ...
    {
        DebugLog::Error(__FUNCTION__, " => GetDevice()->CreateTexture2D() failed.");
        return nullptr;
    }
